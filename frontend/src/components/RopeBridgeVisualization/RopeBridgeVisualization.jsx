...
function SightingMarker({ location, subtitle, onSelectLocation }) {
...
function MarkerTrack({ title, locations, onSelectLocation, subtitle }) {
  const trackRef = useRef(null);
  const barRef = useRef(null);
  const dragRef = useRef(null);
  const [scrollBar, setScrollBar] = useState({ ratio: 1, offset: 0 });
  ...
  const scrollToClientX = (clientX) => {
    ...
    const thumbWidthPx = barRect.width * scrollBar.ratio;
  ...
  return (
    <div className="rope-bridge-marker-track-wrap">
      <div className="rope-bridge-marker-track" ref={trackRef}>
...
function MarkerRow({ title, locations, onSelectLocation, subtitle }) {
  return (
...
function MarkerDeckOverlay({ title, locations, onSelectLocation, subtitle }) {
  return (
...
function BridgeSpan({ id, label, anchorY, postGradientId }) {
...
function BridgeScene({ id, label, anchorY }) {
...
function RopeBridgeVisualization({ locations, onSelectLocation }) {
  const bridgeA = useMemo(() => locations.filter((location) => location.ropeBridgeId === "A"), [locations]);
  const bridgeB = useMemo(() => locations.filter((location) => location.ropeBridgeId === "B"), [locations]);
  ...
  const unassigned = useMemo(
    () => locations.filter((location) => location.ropeBridgeId !== "A" && location.ropeBridgeId !== "B"),
    [locations]
  );
